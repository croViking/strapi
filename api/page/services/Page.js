...
module.exports = {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Page.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Page._attributes)
      .filter(attribute => attribute !== Page.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Page._attributes[attribute].type));
